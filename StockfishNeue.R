...
for(i in 1:length(moves)) {
    ...
    if(!file.exists(paste("./Openings/",opening,"/","Game", as.character(i),"-", opening,"-", handle, ".txt", sep = ""))) {
        write.table(cleanDF(i), quote = FALSE,
                    file = paste("./Openings/",opening,"/","Game", as.character(i),"-", opening,"-", handle, ".txt", sep = ""))}
}
